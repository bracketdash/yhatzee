var app = new Vue({
  ...
  methods: {
    makeRecommendation: function () {
      ...
      if (
        this.dice.length !== 5 ||
        _.intersection(this.diceArr, [7, 8, 9, 0]).length
      ) {
      ...
      if (this.rollsLeft) {
        ...
        _.each(rerollCombos, function (rerollCombo) {
          self.getPotentialRerolls(
            _.map(rerollCombo.split(""), function (val) {
              return parseInt(val, 10) - 1;
            }),
            _.clone(self.diceArr),
            [],
            [],
            function (newPotentialsRerolls) {
      ...
      function finishMakingRecommendation() {
        if (
          rerollComboGroupsArr.length &&
          rerollComboGroupsArr[0].rating > 50
        ) {
          ...
          self.recommendation =
            self.availablePointCombos[0].name +
            " for " +
        ...
        if (self.rollsLeft) {
          self.weightedRerollComboRating =
            rerollComboGroupsArr[0].rating.toFixed(1);
        } else {
    ...
    getPotentialRerolls: function (
      ...
      if (
        sortedPotentialRerolls.length ===
        maxLengthMap[rerollComboArr.length - 1]
      ) {
    ...
    getPointCombos: function (diceArr) {
      ...
      var self = this;
      _.each(
        _.filter(this.pointCombos, { used: false }),
        function (pointCombo) {
